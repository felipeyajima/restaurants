...
public class TableController {
    ...
    public TableDto createTable(@RequestBody TableDto dto){
        Table saved = createTable.createTable(new Table(dto.tableNumber(), dto.numberOfChairs()));
        return new TableDto(saved.getTableNumber(), saved.getNumberOfChairs());
    }

    @GetMapping
    public List<TableDto> listTables(){
        return listTables.getAllTables().stream().map(t -> new TableDto(t.getTableNumber(), t.getNumberOfChairs())).collect(Collectors.toList());
    }
